...
// VerifyIsZero 判断传递的数据是否有初值
func VerifyIsZero(values ...any) bool {
...
// VerifyJsonFiled 判断是否合法的json字段
func VerifyJsonFiled(model any, tag string) bool {
	of := reflect.TypeOf(model)
	for j := 0; j < of.NumField(); j++ {
		if of.Field(j).Tag.Get("json") == tag {
			return true
		}
	}
	return false
